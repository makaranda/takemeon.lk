...
<div class="container mt-5">
    ...
    @endif
  <div class="card">
    <div class="card-header">Gallery Item Content</div>
     <div class="card-body">
            ...
            <div class="col-12 col-md-12">
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col text-end">
                        <a href="{{ route('admin.createevent.items') }}" id="add_blog" name="add_blog" class="btn btn-gray border float-right">Add Event Item</a>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @if($pages)
                            @foreach ($pages as $page)
                                <tr class="action_area">
                                    ...
                                    <td>
                                        <h6>{{ $page->gallery_year }}</h6>
                                        <span>{{ $page->gallery_title }}</span>
                                        <div class="btn_action_area">
                                            {{-- <a href="{{ route('admin.editevent.items',$page->id) }}" target="_self">Edit</a> --}}
                                            <a href="#" class="text-danger page_trash" data-id="{{ $page->id }}">Trash</a>
                                            {{-- <a href="{{ url('events/item/'.$page->slug) }}" target="_blank">View</a> --}}
                                        </div>
                                    </td>
                                    <td>{{ $page->author_id ? $page->author_name : 'No Author' }}</td>
                                    <td></td>
                                    <td>
...
<script>
    ...
    $('.page_trash').on('click', function () {
        ...
        var ajax_url = "{{ route('admin.deleteevent.items', ':id') }}";
