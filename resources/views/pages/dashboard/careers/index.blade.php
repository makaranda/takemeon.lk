            ...
            <div class="col-12 col-md-12">
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col text-end">
                        <a href="{{ route('admin.createcareer') }}" id="add_blog" name="add_blog" class="btn btn-gray border float-right">Add Career</a>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <tr class="action_area">
                                ...
                                <td>
                                    ...
                                    <div class="btn_action_area">
                                        <a href="{{ route('admin.editcareer',$career->id) }}" target="_self">Edit</a>
                                        <a href="#" class="text-danger page_trash" data-id="{{ $career->id }}">Trash</a>
                                        <a href="{{ url('career/'.$career->slug) }}" target="_blank">View</a>
                                    </div>
                                </td>
                                <td>{{ $career->author ? $career->author->name : 'No Author' }}</td>
                                <td></td>
                                <td>
                                    <div class="publish_badge">Published</div>
                                    <div class="date_time">{{ \Carbon\Carbon::parse($career->updated_at)->format('Y/m/d \a\t g:i a') }}</div>
                                </td>
                            </tr>
                        @endforeach
                    @endif
...
<script>
    ...
    $('.page_trash').on('click', function () {
        ...
        console.log("Selected Blog ID: ", page_id);

        var ajax_url = "{{ route('admin.deletecareer', ':id') }}";
        ajax_url = ajax_url.replace(':id', page_id);

        // Call the AlertModelDetails function
        AlertModelDetails(
            'Confirm Deletion',
            'Are you sure you want to delete this Career Item..??',
            'Cancel',
            'Delete',
            page_id,
            ajax_url,
