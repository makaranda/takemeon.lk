...
class AccordingsController extends Controller
{
    // Show all According records
    public function index()
    {
        $accordings = According::where('type','programme')->orderBy('order','asc')->get();
        $programmes = Programme::where('status',1)->get();
        return view('pages.dashboard.programme.accordings', compact('accordings','programmes'));
    }

    // Show create form
    public function create()
    {
        $accordings = According::where('status',1)->where('type','programme')->get();
        $programmes = Programme::where('status',1)->get();
